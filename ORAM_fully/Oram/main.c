...
int memory_access_cycle_I = 2; // access file every 5 circles
int memory_access_cycle_II = 3; 
int memory_access_cycle_III = 4; // access file every 5 circles
int memory_access_cycle_IV = 5; 
...
void Compute_Param(){
    ...
    stage_1 = shuffle_period*0.1666;
    stage_2 = shuffle_period*0.3333;
    stage_3 = shuffle_period*0.6666;
    stage_4 = shuffle_period*0.8333;
}
...
int main(int argc, const char * argv[]) {
    ...
    REQ * request = (REQ*) malloc(memory_access_cycle_IV *sizeof(REQ));
...
// --------------- Stage IV: small amount of tree accesses ---------------------

        if(N>stage_3 && N <=stage_4){
            for(int i=0; i<memory_access_cycle_III; i++){
                tree_access(request[i].data, tree, pos_map, &stash, request[i].tree_address,request[i].op , tree_key);
            }
        }
// --------------- Stage VII: large amount of tree accesses ----------------------
